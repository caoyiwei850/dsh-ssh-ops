...
1. **DSH Desktop 桌面版适配**：检测无边框窗口标题栏，SSH 面板顶端对齐侧栏「新会话」按钮上边沿，避免与系统关闭按钮重叠。
2. **多终端标签页**：多服务器标签 + 标签右侧 `×` 单独断开；删除「关闭终端」按钮；面板顶部 `×` 仅隐藏面板；面板右上角冗余 `＋` 已移除（用标签条 `＋` 添加服务器）。
3. **`ssh_write` 输入后回车**：新增 `press_enter`（默认 `true`），自动补 `\r`（回车 CR），使密码/`[Y/n]` 等 raw 模式提示可提交。
4. **`ssh_write` 指定终端执行**：可传 `connection_id` 指定目标终端（多服务器时明确要写哪台）；目标连接若无已开终端，自动打开一个再写入，不再出现「Sent 0 bytes」空写。
5. **连接对话框「保存并连接」**：临时连接表单可直接保存为 SSH 资源并连接，认证信息写入本机 DSH 凭据库（`profileSave` + `credentials.set`）。
6. **修复**：连接成功后清空面板残留错误提示。
